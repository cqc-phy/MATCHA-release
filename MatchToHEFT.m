...
ClearAll[MatchToHEFTFunctionDefinedQ];
...
MatchToHEFT[
  ...
  {
    eftOrder, simplifyResult,
    exportDiagrams, outputDirectory, requiredFunctions,
    ampLO, ampDiags, ampDiagsForNext, ampUV, ampProcessed, onlyRelevantDiagrams, 
    ampExpanded, ampDirac, ampInvariant, matchingObj, solvedObj, redefinedObj,finalRules, localResults,alternativeBasis, coeffDir, coeffBase, alternativeRules, toJSONRules
  },
  ...
  alternativeBasis = OptionValue[AlternativeBasis];
  ...
  *)
  ...
  If[TrueQ[alternativeBasis],

    alternativeRules =
      CleanSolutionRules[redefinedObj["RedefinitionInputSolutions"]];

    Put[alternativeRules, coeffBase <> "_AlternativeBasis.m"];

    Export[
      coeffBase <> "_AlternativeBasis.json",
      toJSONRules[alternativeRules],
      "JSON"
    ];
  ];
  ...
  finalRules =
    finalRules /. (lhs_Symbol -> rhs_) :>
      With[
        ...
        ToExpression[Context[lhs] <> newName] -> rhs
      ];
  ...
  Module[
        ...
        ! MemberQ[
          {
            "Global`MATCHADiagrams",
            "MATCHADiagrams",
            "Global`MATCHARedefinedMatchingObject",
            "MATCHARedefinedMatchingObject"
          },
          #
        ] &
      ];
    ...
    namesToRemove =
      namesToRemove /. s_String /; ! StringContainsQ[s, "`"] :>
        "Global`" <> s;

    Scan[Remove, namesToRemove];
  ];
