...
MakeRedefinitionObject[obj_Association, opts:OptionsPattern[]] := Module[
  {
    ...
    derivativeSolutions, allInputSolutions, ctx, higgsf, v, mh, Mw, Mq,
    der, mu, hfield, derh, drules, xvars, pvars, dvars, avars, cvars,
    dfinalvars, afinalvars, cfinalvars, htrafo, Lnoncanonic,
   LnoncanonicRedefined, LnoncanonicRedefinedGeneral,
    kineticEquations, kineticEquationsGeneral,
    xSolutions, xSolutionsGeneral, xRules, xRulesGeneral,
  ...
  Mq = Lookup[$SMParams, "QuarkMass", SymbolInContext[ctx, "Mq"]];
  ...
  LnoncanonicRedefined =
    ...
    LnoncanonicRedefinedGeneral = Expand[LnoncanonicRedefined];

  kineticEquationsGeneral =
    Table[
      Coefficient[LnoncanonicRedefinedGeneral, hfield, vark] == 0,
  ...
  xRulesGeneral =
    If[
      xSolutionsGeneral === {} || xSolutionsGeneral === {{}},
      {},
      Simplify[First[xSolutionsGeneral]]
  ];
  ...
  LYukawaList[nhomax_Integer?Positive] :=
    Table[
      Mq Ybil cvars[nhomax][[n]] higgsf^n/v^n,
      {n, 1, nhomax}
    ];
  ...
  Join[
    ...
    <|
      ...
      "FieldRedefinitionRulesGeneral" -> xRulesGeneral,
