...
MatchingEquationsFromExpression[expr_, entry_Association] := Module[
  {n, vars, fulleq, nonder, eqnonder, eqder, equations, sol1, sol2, solutionsboth},
  ...
  sol2 = If[
    eqder =!= {},
    Solve[First[eqder], vars[[2]]],
    {}
  ];

  equations = Join[eqnonder, eqder];

  solutionsboth = Flatten[{sol1, sol2}];

  <|
    "Equations" -> equations,
    "Solution" -> solutionsboth
  |>
];

SolveMatchingProcess[entry_Association, opts:OptionsPattern[]] := Module[
  {expr, vars, solverOutput, equations, solution},
  ...
  If[!KeyExistsQ[entry, "MatchingExpression"],
    ...
    Return[
      Join[
        entry,
        <|"MatchingEquations" -> {}, "MatchingSolution" -> $Failed|>
      ]
    ]
  ];
  ...
  solverOutput = MatchingEquationsFromExpression[expr, entry];

  equations = solverOutput["Equations"];
  solution = solverOutput["Solution"];
  ...
  Join[
    entry,
    <|"MatchingEquations" -> equations, "MatchingSolution" -> solution|>
  ]
];
