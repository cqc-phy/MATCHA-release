  ...
  processed = If[
    ...
    withPackageOutputControl[
      ...
      If[
        ...
        $Failed,
        If[
          MemberQ[{"GaugeHiggsCharged", "GaugeHiggsNeutral"}, family],
          tmp = FC["UnAbbr"][tmp] /. {
...
ProcessUVAmplitudeForProcess[
  ...
  If[
    !KeyExistsQ[entry, "UVAmplitudeRaw"],
    Message[ProcessUVAmplitudeForProcess::missingRawAmplitude];
    Return[
      Join[
        entry,
        <|"UVAmplitudeProcessed" -> $Failed|>
      ]
    ]
  ];
  ...
  processed = If[
    ...
    withPackageOutputControl[
      ...
      If[
        ...
        (* Select the amplitude first and apply UnAbbr only once *)
        tmp = FC["UnAbbr"][tmp[[1]]];

        If[
          MemberQ[
            {"GaugeHiggsCharged", "GaugeHiggsNeutral"},
            family
          ],
          tmp = tmp /. {
            FC["Pair"][
  ...
  Join[
    entry,
    <|"UVAmplitudeProcessed" -> processed|>
  ]
];
